...
\item A program is needed to find the confirmed divergence setups and then inform us to check the currency pare the divergence is accuring on. Requirements:
\begin{enumerate}
\item Get and read (have access to) the data on currenncies for at least the last two month.
\item Use MACD indicator and find out the divergences accures in the pairs lately in given time frames that it told to.
\item Automatically ignore the divergencies that are unlikely to be good enough. Based on the target for example less than 20 pips for each target.
\item Automatically check the MYfxbook to see if the currency lable is red or not for the day. 
\item Inform me if the proper divergence was found due to all the requiremnets it should have. 
